    ...
    <?php if (!empty($forms)): ?>
      <?php foreach ($forms as $i => $form): ?>
        <div class="adoption-card">
          <img class="adoption-thumb" src="<?= htmlspecialchars($form['image_path'] ?? 'public/assets/images/pets.png') ?>" alt="Pet">
          <div class="adoption-details">
            ...
            <div style="font-size:1rem; color:#3b5d50; margin-top:2px;"><i class="fa fa-dog me-1"></i><?= htmlspecialchars($form['pet_name']) ?> <span style="color:#888;">(<?= htmlspecialchars($form['pet_type']) ?>, <?= htmlspecialchars($form['breed']) ?>)</span></div>
          </div>
  ...
  let html = `
    <div class='row g-4 align-items-center mb-4 flex-wrap'>
      <div class='col-md-4 text-center mb-3 mb-md-0'>
        ${form.image_path ? `<img src='${form.image_path}' style='width:140px;height:140px;object-fit:cover;border-radius:50%;border:3px solid #e2e9e8;box-shadow:0 6px 24px rgba(67,108,93,0.18);margin-bottom:12px;'>` : '<span class=\"text-muted\">No image</span>'}
      </div>
      <div class='col-md-8'>
    ...
    <div class='row g-3 mb-4'>
      <div class='col-12 col-md-6 d-flex align-items-center'><span class='text-secondary fw-bold me-2'><i class='fa fa-user-circle me-1'></i>Name:</span> <span style='color:#2563eb; font-weight:600;'>${form.requester_name}</span></div>
      <div class='col-12 col-md-6 d-flex align-items-center'><span class='text-secondary fw-bold me-2'><i class='fa fa-phone me-1'></i>Phone:</span> <a href='tel:${form.phone}' style='color:#3b5d50; text-decoration:underline;'>${form.phone}</a></div>
      <div class='col-12 col-md-6 d-flex align-items-center'><span class='text-secondary fw-bold me-2'><i class='fa fa-envelope me-1'></i>Email:</span> <a href='mailto:${form.requester_email}' style='color:#2563eb; text-decoration:underline;'>${form.requester_email}</a></div>
      <div class='col-12 col-md-6 d-flex align-items-center'><span class='text-secondary fw-bold me-2'><i class='fa fa-map-marker-alt me-1'></i>Address:</span> <span style='color:#444;'>${form.address ?? ''}</span></div>
    </div>
    <div class='mb-4 pb-2' style='border-bottom:1px solid #f3f4f6;'>
      <span class='fw-bold' style='color:#3b5d50;font-size:1.18rem;'><i class='fa fa-info-circle me-2'></i>Request Details</span>
    </div>
    <div class='row g-3 mb-2'>
      <div class='col-12 col-md-6 d-flex align-items-center'><span class='text-secondary fw-bold me-2'><i class='fa fa-calendar-alt me-1'></i>Request Date:</span> <span style='color:#6c757d; font-size:0.98em;'>${form.preferred_date}</span></div>
      <div class='col-12 col-md-6 d-flex align-items-center'><span class='text-secondary fw-bold me-2'><i class='fa fa-home me-1'></i>Home Type:</span> <span style='color:#444;'>${form.home_type ?? ''}</span></div>
      <div class='col-12 col-md-6 d-flex align-items-center'><span class='text-secondary fw-bold me-2'><i class='fa fa-paw me-1'></i>Other Pets?:</span> <span class='badge ${form.has_other_pets ? 'bg-success' : 'bg-secondary'}' style='font-size:1em; padding:4px 14px;'>${form.has_other_pets ? 'Yes' : 'No'}</span></div>
      <div class='col-12 col-md-6 d-flex align-items-center'><span class='text-secondary fw-bold me-2'><i class='fa fa-comment-dots me-1'></i>Reason:</span> <span class='badge bg-primary' style='font-size:1em; padding:4px 14px; background:#e6f0fa !important; color:#2563eb !important;'>${form.reason}</span></div>
    </div>
  `;
